...
String[][] textosCreate(byte[] tex_bytes)
{
	int dataPos = 0;
	int dataBak = 0;
	...
	// MONGOFIX ==========================================
	char textos[] = new char[tex_bytes.length];
	for(int i = 0; i < tex_bytes.length; i++)
		if(tex_bytes[i] < 0)
			textos[i]=(char)(tex_bytes[i]+256);
		else
			textos[i]=(char)tex_bytes[i];
	//=================================================
...
{		
    if(sCheat[iCheat] == -6){cheatActive = !cheatActive;iCheat = 0;}
    if (keyMisc == 0) return;
    //System.out.println(""+iCheat+" - "+keyMisc);		
	if (sCheat[iCheat] == keyMisc && lastKeyMisc == 0) iCheat++;
...
//	if (!menuList_ON || !menuListUpdate) {return;}
	...
	gc.canvasFillShow = menuFillShow;
	gc.canvasFillRGB = menuFillRGB;
	...
	case ML_OPTION:
		...
		int y = menuListY + (( menuListSizeY / 4) * 3) - ((texto.length * menuListDat[menuListPos][4])/2);
...
public void menuListTextDraw(Graphics g, int format, String str, int x, int y)
{
	...
	g.setColor(0);
	g.setClip(0, 0, gc.canvasWidth, gc.canvasHeight); //ZNR
