...
{	 
   ...
   int anim = 0;
	...
	public malo4(Image i, int w, int h, int numf, int g, int j)
	{	
	...
	int fireanim[][] ={{1,1,0,0,0,0,1,1,2,2},
							{1,1,0,0,0,0,1,1,2,2},
							{1,1,0,0,0,0,1,1,2,2},
							{1,1,2,2,2,2,1,1,0,0}		
							};	
	   ...
	   {	   
	   	...
	   	if (disparando)
		   {
		   	...
		   	{	
		   		switch (ashape-4)
		   		{
		   			case 0:  if (Game.GetTile(x+(width/2),y+(height)) == 230) 
		   			         {
		   			            Game.SetTile(x+(width/2),y+height,2); 
		   			            Game.jode(x+(width/2),y+height); 
		   			         }   
		   						break;
		   			case 1:  if (Game.GetTile(x+(width/2),y+8) == 230) 
		   			         {
		   			            Game.SetTile(x+(width/2),y+8,2); 
		   			            Game.jode(x+(width/2),y+8); 
		   			         }   
		   						break;
		   			case 2:  if (Game.GetTile(x+width,y+20) == 230) 
		   			         {
		   			            Game.SetTile(x+width,y+20,2); 
		   			            Game.jode(x+width,y+20); 
		   			         }
		   						break;
		   			case 3:  if (Game.GetTile(x-3,y+20) == 230) 
		   			        {
		   			            Game.SetTile(x+3,y+20,2); 
		   			            Game.jode(x+3,y+20); 
		   			         }
		   						break;
		   		}
		   ...
		   else
		   {
	   		...
	   		if (timing%2 == 0)
	   		{
				...
				if (timing > 30) timing = 0;
				if (timing == 20  && (Math.abs(x - prota.x) < 64)  && (Math.abs(y - prota.y) < 64))	
				{
					disparando = true;
					desp = 0;	
	   			ashape += 4;   	
