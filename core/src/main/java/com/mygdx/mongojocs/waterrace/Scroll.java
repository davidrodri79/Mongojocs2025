...
public class Scroll {
	...
	public void ScrollINI(int SizeX, int SizeY) {
		...
		Gdx.app.postRunnable(new Runnable() {
			...
			public void run() {
				FondoImg = new Image();
				FondoImg._createImage(FondoSizeX*12, FondoSizeY*12);
	...
	public void ScrollSET(byte[] Mapa, int SizeX, int SizeY,  Image Img, int LineX) {
		...
		for (int i=0 ; i<FondoMap.length ; i++) {
			FondoMap[i]=-1;
		}
	...
	public void ScrollRUN(int X, int Y) {
		...
		while (X>=FaseSizeX*12) {
			X-=(FaseSizeX*12);
		}
		while (Y>=FaseSizeY*12) {
			Y-=(FaseSizeY*12);
		}
	...
	public void ScrollRUN_Max(int X, int Y) {
		...
		if (X<0) {
			X=0;
		} else if (X>=(FaseSizeX*12)-ScrollSizeX) {
			X=(FaseSizeX*12)-ScrollSizeX;
		}
		if (Y<0) {
			Y=0;
		} else if (Y>=(FaseSizeY*12)-ScrollSizeY) {
			Y=(FaseSizeY*12)-ScrollSizeY;
		}
	...
	public void ScrollRUN_Centro_Max(int X, int Y) {
		ScrollRUN_Max(X-(ScrollSizeX/2), Y-(ScrollSizeY*2/3));
	}
