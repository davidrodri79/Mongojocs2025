...
public void ScrollINI(int SizeX, int SizeY)
{
	...
	FondoImg=new Image();
	FondoImg._createImage(FondoSizeX*8, FondoSizeY*8); //MONGOFIX
...
{
	int colo = 24+(Trig.sin(Game.nfc>>3)>>6);
	...
	if (y==FondoY) {if ((CorY-=FondoSizeY) < 0) {CorY+=FaseSizeY;}}
		...
		for (int x=0, x2=FondoX, i=0 ; i<2; i++)
		{
			for (; x<x2 ; x++)
			{
				...
				if (Game.fase == 1)
				{
			   //FondoGfx.setColor(10+(y<<3),33-(y)+(colo>>1),0);		
			   FondoGfx.setColor(128-(Math.abs(8-y)<<4),33-(y)+(colo>>1),0);		
				//FondoGfx.setColor(30+(y*4),colo+33-(y*2),colo+30-y);		
				//FondoGfx.setColor(colo+(y*4),colo-(y*2),colo-y);		
