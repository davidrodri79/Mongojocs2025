...
// -------------------------------------
// Marco Class v1.5 - Rev.5 (8.7.2003)
// =====================================
...
public void MarcoFIX(int Dato, String Texto)
{
	...
	try {
		...
		MarcoADD(Dato, Texto.substring(PosIni, (PosOld >= Texto.length() ? Texto.length() - 1 : PosOld)));
	} catch (Exception e)
	{
		e.printStackTrace();
	}
...
public int MarcoRUN(int Key, int KeyY)
{
	...
	case 2:
		...
		if (KeyY!=-1 && (MarTexScrY-=MarTexDesp) < -TextoSizeY) {Ret=-2;}
		}
	break;
