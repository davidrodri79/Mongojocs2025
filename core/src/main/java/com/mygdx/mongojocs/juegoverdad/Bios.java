...
public void run()
{
	...
	while (!gameExit) {
		if (!gamePaused || ((Game)this).inputCalled) {
			...
			GameSleep=(FRAMETIME-(int)(System.currentTimeMillis()-GameMilis));
...
// RND - Engine
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
private long RNDSeed = (System.currentTimeMillis() * 0x5DEECE66DL + 0xBL) & ((1L << 48) - 1);

public int RND(int Max) {
	RNDSeed = (RNDSeed * 0x5DEECE66DL + 0xBL) & ((1L << 48) - 1);
	return (int)(RNDSeed%Max);
}

public void RNDSet() {
	RNDSeed = (RNDSeed ^ System.currentTimeMillis() ^ 0x5DEECE66DL + 0xBL) & ((1L << 48) - 1);
}
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
// --------------------
	case 22:
		if ( menuTick() ) { biosStatus = 0; ((Game)this).restoreSKState(); }
	return;
// --------------------
	}
}
...
{
	int yP = gc.xPressed + gc.yPressed;
	int skP = gc.skPressed;
	int fP = gc.fPressed;
	gc.fPressed = gc.skPressed = gc.xPressed = gc.yPressed = 0;
	if (yP >  1) yP =  1;
	if (yP < -1) yP = -1;


	if ( (menuListMode == ML_SCROLL || menuListMode == ML_SCREEN) && (keyMenu == 0) && (skP != 0)) {
		menuAction(-2);
	} else {
		if ( menuListTick( yP, (skP != 0) || (fP != 0) ) )	{
...
public void menuListAdd(int Dato, String Texto)
{
	...
	if ( GameCanvas.font.stringWidth(Texto) <= menuListSizeX)
	{
...
public void menuListAdd(int Dat0, String[] Texto, int Dat1, int Dat2)
{
	...
	int TextoSizeX = 8;
	for (int SizeX, t=0 ; t<Texto.length ; t++) {SizeX=GameCanvas.font.stringWidth(Texto[t]); if ( TextoSizeX < SizeX ) {TextoSizeX=SizeX;}}


	Str[i] = Texto;
	Dat[i] = new int[] {Dat0, Dat1, Dat2, TextoSizeX, GameCanvas.font.getHeight() };
...
//#endif
	g.setColor(0xFFFFFF);
	g.fillRect(0, 0, gc.canvasWidth, gc.canvasHeight);
...
public String[] menuListCutText(int Dato, String Texto)
{
	...
	while ( Size < (menuListSizeX) )
	{
		...
		Size += GameCanvas.font.stringWidth(new String(new char[] {(char)Dat}));
	}
...
//#endif
	...
	public final static int FRAMETIME = 1000 / FPS;
}
