...
String[][] textosCreate(byte[] tex)
{
	...
	// MONGOFIX ==========================================
	char textos[] = new char[tex.length];
	for(int i = 0; i < tex.length; i++)
		if(tex[i] < 0)
			textos[i]=(char)(tex[i]+256);
		else
			textos[i]=(char)tex[i];
	//=================================================
...
public boolean logosTick()
{
	...
	gc.canvasImg = Image.createImage("/Logo"+cntLogos+".png");
	cntLogos++;
...
public void menuListDraw(Graphics g)
{
	...
	case ML_OPTION:
		...
		int y = menuListY + (( 2*menuListSizeY / 3) * 1) - ((texto.length * menuListDat[menuListPos][4])/2);
