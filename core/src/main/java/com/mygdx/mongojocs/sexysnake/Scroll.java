...
public class Scroll {
	...
	public static Scroll instance = new Scroll();
	// -------------------
	// Scroll Constructor
	// ===================
	...
	public void ScrollRUN_Max(int X, int Y) {
		...
		if (X<0) {
			X=0;
		} else if (X>=(FaseSizeX*8)-ScrollSizeX) {
			X=(FaseSizeX*8)-ScrollSizeX;
		}
		if (Y<0) {
			Y=0;
		} else if (Y>=(FaseSizeY*8)-ScrollSizeY) {
			Y=(FaseSizeY*8)-ScrollSizeY;
		}
