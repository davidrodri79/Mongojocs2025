...
public class MMenu
{
	...
	public int show(Graphics gr, int currentkey, boolean apretando, int w, int h)
	{
		...
		switch(estado)
           ...
           case 4:                                           
                  ...
                  //Flechas(h/2);
                  Game.SetPrint(0xffff00, w/2, MENULINE+((h-128)/2),GHCENTER);        
                  gr.setColor(0x400040);
                  gr.fillRect(0, MENULINE+((h-128)/2), w, 12);
                  ...
                  if (apretando)
                  {
                  	    if (currentkey== Display.KEY_LEFT || currentkey==Display.KEY_UP) if (opmenu > 0) {opmenu--;}else opmenu = lll[prestado].length-1;//Game.currentkey=0;}
	                    if (currentkey==Display.KEY_RIGHT || currentkey==Display.KEY_DOWN)
	                    ...
	                    {                         
	                        switch(lll[prestado][opmenu])
	                        {
        	                    ...
        	                    case ID_MENU:
        	                        ...
        	                        estado = 4;Game.prestado=1;break;
        	                    case ID_QUIT:
        	                        estado=6;break;
        	                    case ID_HELP: 
        	                        estado=5;break;
               ...
               case 5:
           	   ...
           	   case	8:
           	        ...
           	        //gr.setColor(0);gr.fillRect(0,h/2, w, 14);
           	        Game.joc.Pinta();
           	        if (Game.joc.pacman.desp >= 4) Game.joc.pacman.desp -= 4;
           	        else Game.joc.pacman.desp += 4;     
                    Game.SetPrint(w/2, MENULINE+((h-128)/2),GHCENTER);        
	...
	int wmop;
 	 ...
 	 public void Wop(int max)
 	{
 	    ...
 	    if (Game.currentkey==Display.KEY_UP) wmop--;
	    if (Game.currentkey==Display.KEY_DOWN) wmop++; 	                   
 	    if (Game.currentkey==Display.KEY_LEFT) wmop--;
	    if (Game.currentkey==Display.KEY_RIGHT) wmop++; 
	    if (wmop < 0) wmop = max;
	    if (wmop > max) wmop = 0;	                    
	    Game.currentkey = 0;
 	}
