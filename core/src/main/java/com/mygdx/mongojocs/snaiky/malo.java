...
public class malo extends unit
{
    ...
    {                        
        ...
        if (y%4 == 0 && TestCol(x+ cix+incx, x+cfx-1+incx, y+ciy, y+cfy-1, AIR)) x += incx;
        else incx = 0;
        if (x%4 == 0 && TestCol(x+ cix, x+cfx-1, y+ciy+incy, y+cfy-1+incy, AIR)) y += incy;
        else incy = 0;    
            ...
            if (estado == 1)
            {
                 if (rub > 0)
                 {
                    for(int i=x/4; i < (x+width)/4;i++)
                        ...
                        if (Test(i*4,j*4, 0) && Game.rnd.nextInt() > 0) {Set(i*4,j*4, 12+Math.abs(Game.rnd.nextInt()%4));rub--;Game.nblocks++;}
                  }
            }
            else  
            {
                 for(int i=x/4; i < (x+width)/4;i++)
                    for(int j=y/4; j < (y+height)/4;j++)     
                        if (Test(i*4,j*4, TRASH) && Game.rnd.nextInt() > 0) {Set(i*4,j*4, 0);rub++;Game.nblocks--;}
            }
        }
